    ...
    public void render(MinecraftClient client, MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++) {
                ...
                if (selected || active) {
                    drawColoredQuad(matrices.peek().getPositionMatrix(), x, y, active ? selected ? 0x444444 : 0x000000 : 0xAAAAAA);
                }
            }
        }
        matrices.pop();
        ...
        if (accessor.getRecipe() == null){
            ...
            for (int x = 0; x < 8; x++) {
                for (int y = 0; y < 8; y++) {
                    boolean active = accessor.getPixel(x, y) > 0;
                    if (active) {
                        drawColoredQuad(matrices.peek().getPositionMatrix(), x, y, 0x000000);
                    }
                }
            }
