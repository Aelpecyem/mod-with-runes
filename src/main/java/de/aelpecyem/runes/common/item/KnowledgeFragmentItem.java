...
public class KnowledgeFragmentItem extends Item {
    ...
    public void setKnowledge(ItemStack stack, RuneEnchantingRecipe recipe){
        ...
        stack.getNbt().putString("RunesRecipe", recipe.id().toString());
        stack.getNbt().put("RunesRecipeOutput", recipe.output().writeNbt(new NbtCompound()));
    }

    public RuneEnchantingRecipe getKnowledge(ItemStack stack){
        if (!stack.hasNbt() || !stack.getNbt().contains("RunesRecipe")){
            return null;
        }
        Identifier id = new Identifier(stack.getNbt().getString("RunesRecipe"));
        return RuneEnchantingRecipe.recipes.get(id);
    }

    public ItemStack getRecipeStack(ItemStack stack){
        if (!stack.hasNbt() || !stack.getNbt().contains("RunesRecipeOutput")){
            return ItemStack.EMPTY;
        }
        return ItemStack.fromNbt((NbtCompound) stack.getNbt().get("RunesRecipeOutput"));
    }
