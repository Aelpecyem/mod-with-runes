...
public class OrbOfKnowledgeItem extends TrinketItem {
    ...
    public static int storeXP(int amount, ItemStack stack){
        ...
        if (spaceLeft < 0){
            stack.getNbt().putInt("StoredXP", MathHelper.clamp(stored + amount + spaceLeft, 0, getMaxStoredXP(stack)));
            return -spaceLeft;
        }
        stack.getNbt().putInt("StoredXP", stored + amount);
        return 0;
    }
    ...
    public static int extractXP(ItemStack stack){
        ...
        int xp = stack.getNbt().getInt("StoredXP");
        if (xp > 0){
            stack.getNbt().putInt("StoredXP", xp - 1);
            return 1;
        }
        return 0;
    }
    ...
    public static int getStoredXP(ItemStack stack){
        return stack.hasNbt() ? stack.getNbt().getInt("StoredXP") : 0;
    }
