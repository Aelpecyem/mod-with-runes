...
public class RaidhoRuneItem extends Item implements UseEntityCallback{
    public RaidhoRuneItem() {
        ...
        DispenserBlock.registerBehavior(this, (pointer, stack) -> {
             List<Entity> list = pointer.getWorld().getEntitiesByClass(Entity.class, new Box(pointer.getPos().offset(pointer.getBlockState().get(DispenserBlock.FACING))), EntityPredicates.EXCEPT_SPECTATOR);
